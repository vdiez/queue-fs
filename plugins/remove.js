...
module.exports = (actions, config) => {
    if (!actions.hasOwnProperty('remove')) {
        actions.remove = (file, params) => {
            ...
            //return connection.stat(source, source_params).then(() => connection.remove(source, source_params))
            return connection.remove(source, source_params)
        };
    }
    return actions;
};
