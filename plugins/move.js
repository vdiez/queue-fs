            ...
            return source_connection.stat(source, source_params)
                            ...
                            .catch(err => {
                                if (err) {
                                    if (err.is_directory) {
                                        ...
                                        config.logger.info("Local move not implemented on protocol " + source_params.protocol + ". Trying different connections.");
                                    }
                                    else if (err.file_exists) {
                                        config.logger.info(target + " already exists");
                                        done = true;
                                        return source_connection.remove(source, source_params).catch(err => config.logger.error("Could not remove source after move: ", err));
                                    }
                ...
                .then(() => {
                    ...
                    return target_connection.stat(target, params).catch(() => {})
                        ...
                        .then(() => source_connection.remove(source, source_params).catch(err => config.logger.error("Could not remove source after move: ", err)))
                        .then(() => {
