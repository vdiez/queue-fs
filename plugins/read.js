...
module.exports = (actions, config) => {
    if (!actions.hasOwnProperty('read')) {
        actions.read = (file, params) => {
            ...
            //return connection.stat(source, source_params).then(() => connection.read(source, source_params))
            return connection.read(source, source_params);
        };
    }
    return actions;
};
